...
Inside a Conductor workspace, `bin/dev` automatically uses the workspace's allocated port range, starts an isolated Docker Compose project for PostgreSQL and Redis, and prepares the database. This prevents parallel workspaces from connecting to or restarting each other's databases. Open the app on the workspace's `CONDUCTOR_PORT` instead of assuming port 3000.

Outside Conductor, start the fixed-port services first with `docker compose up -d`, run `bin/rails db:prepare` on first setup, and then run `bin/dev`.
