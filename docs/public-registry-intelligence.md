...
Research verified on **9 September 2026**. This note records the product and implementation boundary; it is not legal advice.
...
| Registry Agency — Commercial Register / NPLE Register | The Registry Agency offers the complete database/structured parts and automated subscriptions as paid specialized services, after application, technical review, privacy review, and contract. The published terms describe XML delivery. No comprehensive public `data.egov.bg` dataset was found that can substitute for the current company record. | `CommercialRegistry::UnavailableProvider` is the default. `CommercialRegistry::ContractProvider`, a strict payload normalizer, and importer are ready for the approved feed. The public portal is not scraped and BULSTAT monthly extracts are not misrepresented as Commercial Register data. | Access and reuse are contract-dependent; published automated-service terms prohibit improper onward provision of the supplied information. |
